...
# (0, 90, 180, 270)
rays_embeddings = model.prepare_default_rays(device=device)
...
def process(cfg: Options, path):
    ...
    with torch.no_grad():
        ...
        # save gaussians
        model.gs.save_ply(gaussians, os.path.join(cfg.workspace, name + '.ply'))
        ...
        imageio.mimwrite(os.path.join(cfg.workspace, name + '.mp4'), images, fps=30)

assert cfg.test_path is not None
if os.path.isdir(cfg.test_path):
    file_paths = glob.glob(os.path.join(cfg.test_path, "*"))
else:
    file_paths = [cfg.test_path]
